import Link from "next/link"
import Image from "next/image"
import { BookButton } from "../ui/book-button"

export function QuoteModuleSection() {
  return (
    <section className="py-16 md:py-24 bg-[#F8F5F0]">
      <div className="container mx-auto px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="rounded-xl overflow-hidden shadow-xl flex flex-col md:flex-row">
            {/* Left content side */}
            <div className="bg-[#262F3F] text-white p-8 md:p-12 flex flex-col justify-center md:w-1/2">
              <h3 className="text-3xl md:text-4xl font-mazzard font-semibold mb-6">Get a quote in minutes</h3>
              <p className="text-gray-300 mb-8 text-lg">
                Starting a quote is the fastest way to see accurate pricing for your specific listing or project. Once you're ready, log in or create an account to place your order.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                <BookButton href="/quote" size="lg" className="bg-white text-primary hover:bg-gray-100">
                  Get quote
                </BookButton>
              </div>
            </div>

            {/* Right image side */}
            <div className="md:w-1/2 relative bg-[#262F3F] flex items-center justify-center hidden md:block">
              <div className="h-full w-full flex items-center justify-center p-6">
                <Image
                  src="/images/quote-screenshot-placeholder.png"
                  alt="Quote process screenshot"
                  width={600}
                  height={350}
                  className="object-contain rounded-lg shadow-lg border border-white/10"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
} 